...
const Signin = ({onRouteChange}) => {
    ...
    // super(props);     this.state = {       signInEmail: '',       signInPassword:
    // ''     }   }   onEmailChange = (event) => {     this.setState({signInEmail:
    // event.target.value})   }   onPasswordChange = (event) => {
    // this.setState({signInPassword: event.target.value})   }   onSubmitSignIn = ()
    // => {     fetch('http://localhost:3000/signin', {       method: 'post',
    // headers: {'Content-Type': 'application/json'},       body: JSON.stringify({
    //       email: this.state.signInEmail,         password:
    // this.state.signInPassword       })     })       .then(response =>
    // response.json())       .then(user => {         if (user.id) {
